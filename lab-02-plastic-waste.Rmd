...
ANSWER: There will be :
  ...
  -Editor: This will be where all the programming place
  -Environment: for git push and track the history of the request from console and keep track of the dataset
  -Console: to run the code from editor
  -files,plots,packages,help,viewer,presentation: all the files and packages are placed here
...
ANSWER: It mean there was no data/variable for that observation. 
...
ANSWER:
  its high consumption and production of single-use plastics, coupled with inadequate waste management infrastructure and insufficient recycling facilities, particularly for PET.
...
```{r plastic_waste_per_cap-max1, eval=TRUE}
library(ggplot2)

ggplot(data=plastic_waste, aes(plastic_waste_per_cap))+
  geom_histogram(binwidth = 0.2) +
  facet_grid(~continent)
```
ANSWERS:
- So far, top 3 continents are Europe, North America and Asia has the highest count of plastic waste per capital. South America has the least plastic waste.
...
```{r plastic_waste_per_cap-max2, eval=TRUE}
library(ggplot2)
...
ANSWER:
- because we want the transparency happened equally throughout the geom and not based on the variables of the dataset. Hence, if we defined alpha in the mapping aesthetics it will be based on a variables so that the transparency will not be equal. 
...
``````{r plastic_waste_per_cap-violinbox, eval=TRUE}
ggplot(data = plastic_waste, 
       mapping = aes(x = continent, 
                     y = plastic_waste_per_cap)) +
  geom_violin()
...
``````
ANSWERS:
  Box plot give: the summary of the given dataset ( summary, mean, median,...)
  
  Violin plot give: show details of the data, show smoothly entire range of the data. 
...
ANSWER: 
- I would say that the mismanaged plastic waste are growing faster than the collected plastic waste per capital. Because based on the plot we can see the trendline that growing up much higher for mismanaged plastic waste than whatever amount from plastic waste. 
...
``````{r plastic_waste_per_cap-violinbox_withColor, eval=TRUE}
ggplot(data = plastic_waste, 
       mapping = aes(x = continent, 
                     y = plastic_waste_per_cap,
                     color = continent)) +
...
``````
ANSWER: 
- I can see the least will be from Europe. South America has some capital that have really high mismanaged plastic waste as well as the plastic waste. On the other hand, Africa has alot of amount of mismanaged plastic waste and the highest will be from Asia. 
...
ANSWER:
- both of the plot look similar alike and there are no distinct difference between them. 
...
```
ANSWER: 
- Based on the plot, I can tell that there is an uptrend from the population that live by the coastal and the amount of plastic waste. There are alot of on plastic waste from Africa shoreline, following up is from the Asia and North America. 
